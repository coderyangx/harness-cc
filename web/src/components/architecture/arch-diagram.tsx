...
const typedVersionsData = versionsData as VersionIndex;
...
function getLayerColorClasses(versionId: string): {
  ...
  const v =
    typedVersionsData.versions.find((v) => v.id === versionId) as { layer?: string } | undefined;
  const layer = v?.layer;
  switch (layer) {
    case "tools":
      return {
...
function collectClassesUpTo(
  ...
  const { versions, diffs } = typedVersionsData;
...
function getNewClassNames(version: string): Set<string> {
  const diff = typedVersionsData.diffs.find((d) => d.to === version);
  if (!diff) {
    const v = typedVersionsData.versions.find((ver) => ver.id === version);
...
export function ArchDiagram({ version }: ArchDiagramProps) {
  ...
  const versionData = typedVersionsData.versions.find((v) => v.id === version);
